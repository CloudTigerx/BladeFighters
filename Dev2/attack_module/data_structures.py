...
# Lightweight DTO for external-facing payloads (service facade)
@dataclass(slots=True)
class Payload:
    """
    Minimal payload descriptor used by higher-level services and modes, without
    exposing internal calculator/engine types.
    """
    kind: Literal["garbage", "strike"]
    count: int
    pattern: Optional[str]  # e.g., "2x6_vertical"; None for garbage
    color: Optional[str]    # reserved for future use; None today
    target_board: Optional[int]  # 1 or 2 when applicable
