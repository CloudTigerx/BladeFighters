...
class InputTunerOverlay:
    ...
    PARAM_KEYS: List[Tuple[str, str]] = [
        ("repeat_initial_delay_ms", "Initial Delay (ms)"),
        ("repeat_interval_ms", "Generic Repeat (ms)"),
        ("repeat_move_interval_ms", "Move Repeat (ms)"),
        ("repeat_rotate_interval_ms", "Rotate Repeat (ms)"),
    ]
    ...
    def _clamp_and_apply(self, key: str, value: int) -> int:
        # Persist via settings service (ConfigService clamps on update)
        try:
            self.settings.update({key: int(value)})
        except Exception:
            pass
        # Reflect values live into input handler
        try:
            if key == 'repeat_initial_delay_ms':
                self.input_handler.key_repeat_delay = int(self.settings.get('repeat_initial_delay_ms', value))
            elif key == 'repeat_interval_ms':
                self.input_handler.key_repeat_interval = int(self.settings.get('repeat_interval_ms', value))
            elif key == 'repeat_move_interval_ms':
                self.input_handler.arrow_repeat_interval = int(self.settings.get('repeat_move_interval_ms', value))
            elif key == 'repeat_rotate_interval_ms':
                self.input_handler.rotate_repeat_interval = int(self.settings.get('repeat_rotate_interval_ms', value))
        except Exception:
            pass
        # Return the potentially clamped value from settings
        try:
            return int(self.settings.get(key))
        except Exception:
            return int(value)

    def _adjust_selected(self, delta: int):
        key, _label = self.PARAM_KEYS[self._selected_index]
        cur = int(self.settings.get(key, 0))
        new_val = self._clamp_and_apply(key, cur + delta)
        self._last_adjust_ts = int(self.clock.now_ms())
        return new_val
