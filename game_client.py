...
# Import the new simplified settings UI
from simple_settings_ui import SimpleSettingsUI
...
class GameClient:
    def __init__(self):
        ...
        self.settings_ui = None  # New simplified settings UI
        self.test_mode = None
    ...
    def _initialize_settings_system(self):
        """Initialize the simplified settings UI."""
        self.settings_ui = SimpleSettingsUI(self.screen, self.font, self.audio, self)
        print("✅ Simplified settings UI initialized")
    
    def _initialize_test_mode(self):
        """Initialize the test mode."""
        self.test_mode = TestMode(self.screen, self.font, self.audio, self.asset_path, self.settings_ui)
    ...
    def get_brightness(self) -> float:
        ...
        if hasattr(self, 'settings_ui') and self.settings_ui:
            return self.settings_ui.settings.get('brightness', 1.0)
    ...
    def get_mouse_sensitivity(self) -> float:
        ...
        if hasattr(self, 'settings_ui') and self.settings_ui:
            return self.settings_ui.settings.get('sensitivity', 1.0)
        return 1.0  # Default sensitivity
    
    def toggle_vsync(self):
        """Working V-Sync toggle that actually changes the display mode."""
        try:
            # Get current V-Sync state from new settings UI
            current_vsync = self.settings_ui.settings.get('vsync', True) if hasattr(self, 'settings_ui') else True
            
            # Toggle the setting
            new_vsync = not current_vsync
            if hasattr(self, 'settings_ui'):
                self.settings_ui.settings['vsync'] = new_vsync
        ...
        try:
            ...
            while self.game_running:
                ...
                if self.current_screen == "main_menu":
                    ...
                    # Draw the simplified settings UI
                    if hasattr(self, 'settings_ui') and self.settings_ui:
                    ...
                    for event in events:
                        if event.type == pygame.MOUSEBUTTONDOWN:
                            ...
                            if hasattr(self, 'settings_ui') and self.settings_ui:
                                if self.settings_ui.handle_click(event.pos):
                                    # If settings UI handled the click, don't process further
                                    continue
                            ...
                            # Handle music controls using keyboard shortcuts
                            if hasattr(self, 'audio') and self.audio:
