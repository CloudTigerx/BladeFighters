...
class GameClient:
    def __init__(self):
        ...
        # First try to find resolutions that fit the desktop
        fitting_resolutions = []
        for res in self.resolutions:
            if res[0] <= desktop_width and res[1] <= desktop_height:
                fitting_resolutions.append(res)
        
        if fitting_resolutions:
            # Use the largest fitting resolution
            self.width, self.height = fitting_resolutions[-1]
        else:
            # If no resolutions fit, try to use the highest available (for monitors that can scale)
            print(f"⚠️  No resolutions fit desktop, trying highest available")
            self.width, self.height = self.resolutions[-1]
            print(f"✅ Selected highest resolution: {self.width}x{self.height}")
        
        print(f"🎮 Final game resolution: {self.width}x{self.height}")
    ...
    def draw_placeholder_settings(self):
        ...
        background_image = pygame.image.load("puzzleassets/magic/rainbackdrop.png")
        scaled_background = pygame.transform.smoothscale(background_image, (self.width, self.height))
