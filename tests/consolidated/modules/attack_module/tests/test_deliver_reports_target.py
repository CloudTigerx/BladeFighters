...
def test_deliver_returns_target_board_and_count(monkeypatch):
    ...
    clock.advance(1200)
    # Call update to process the attack queue and make attacks ready
    svc.attack_manager.update(clock.now_ms() / 1000.0)
    print(f"Player 1 attacks: {len(svc.attack_manager.player1_attacks)}")
    print(f"Player 2 attacks: {len(svc.attack_manager.player2_attacks)}")
